...
- **Executable coverage** = the same, counting only waypoints the arm can
  actually strike (collision-aware IK, best yaw about the surface normal). This
...
2. Orientation: tool z aligned to the surface normal, **yaw searched** (free pad
   DOF) — the yaw maximizing reachable waypoints is used (counter best yaw 150°).
3. Per-waypoint collision-aware IK → reachability mask (honest metrics).
4. Build the joint trajectory over the **longest contiguous reachable stroke**,
   seeded from a start IK state so `compute_cartesian_path` begins on the surface
   (retry handles IK non-determinism). Cartesian fraction achieved: **100 %**.
